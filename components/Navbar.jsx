...
const Navbar = () => {
  ...
  return (
    <>
      <div className={`hidden lg:flex justify-end mr-4 xl:mr-36 sticky top-11 -mt-16 z-50
      ...
      <div className='w-screen flex justify-end lg:hidden fixed top-4 left-0 z-50'>
        <button className={`z-10 mr-4 transition ${showButton() ? '' : '-translate-y-[100px]'} shadow-lg bg-white rounded-full`}>
          <HiOutlineMenu
            size={48}
            color='#8b431e'
        ...
        <nav
          className={`absolute -top-4 right-0 min-h-screen w-screen h-[500px] bg-primary-100 transition ${isOpen ? '' : 'translate-x-[100%]'} flex justify-center items-center short:items-start short:pt-8 overflow-auto`}
        >
          <ul className='flex flex-col justify-center items-center gap-8'>
            ...
            </Link>
            {
              links.map(({ href, title, cta }) => (
                <li
                  key={href}
                  className={`${cta ? 'text-primary-700 font-bold' : 'text-primary-500'} text-xl sm:text-2xl font-semibold group relative overflow-hidden`}
                >
